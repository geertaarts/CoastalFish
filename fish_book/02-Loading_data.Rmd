...
# 
if (is.element("depth_raster.rdata",dir("data/depth/Emodnet_DTM2018/"))){
  load("data/depth/Emodnet_DTM2018/depth_raster.rdata")}

if (is.element("depth_raster.rdata",dir("data/depth/Emodnet_DTM2018/"))==FALSE){

# What is the cellsize?
  val<-raster(paste("E:/GitHub/CoastalFish/fish_book/data/depth/Emodnet_DTM2018/C4_2018.asc",sep=""))
...
# Extract values  
  ...
  for (i in tiles){
    print(i)
    val<-raster(paste("E:/GitHub/CoastalFish/fish_book/data/depth/Emodnet_DTM2018/",i,"_2018.asc",sep=""))
...
# Calculate means of depth  
  ...
  M.raster<-raster(M,xmn=min(xy$x), xmx=max(xy$x), ymn=min(xy$y), ymx=max(xy$y))
    ...
    # Write.table
  save(M.raster,file="E:/GitHub/CoastalFish/fish_book/data/depth/Emodnet_DTM2018/depth_raster.rdata")
...
```{r tpi, echo=TRUE, eval=T, dpi=300, fig.width=6, fig.height=6}
  ...
  tpi_depth<-tpi(M.raster, scale = 15, win = "rectangle", normalize = FALSE)
  plot(tpi_depth)
  plot(land.utm,add=TRUE,border="grey75",col="grey90")
  box(lwd=2)  
```
...
par(mfrow=c(2,2),mai=c(0.1,0.1,0.2,0.2)) 
  slibNL<-raster(file.path(path,"sediment/ascii/slib_juli2007.asc"))
    projection(slibNL)<-CRS("+init=epsg:32631")
    plot(slibNL,xaxt="n",yaxt="n",main="slib")
    plot(land.utm,add=TRUE,border="grey75",col="grey90")
    box()
    
    zand<-raster(file.path(path,"sediment/ascii/zand_afgeleid.asc"))
    projection(zand)<-CRS("+init=epsg:32631")
    plot(zand,xaxt="n",yaxt="n",main="zand")
    plot(land.utm,add=TRUE,border="grey75",col="grey90")
    box()
    
    dz10<-raster(file.path(path,"sediment/ascii/dz10_juli2007.asc"))
    projection(dz10)<-CRS("+init=epsg:32631")
    plot(dz10,xaxt="n",yaxt="n",main="dz10")
    plot(land.utm,add=TRUE,border="grey75",col="grey90")
    box()
    
    dz60<-raster(file.path(path,"sediment/ascii/dz60_juli2007.asc"))

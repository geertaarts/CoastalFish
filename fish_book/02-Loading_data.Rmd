...
###############################
rm(list=ls());
#.libPaths("D:/WMR/Rpackages"); 
#library(vmstools);
library(rgdal);library(raster);library(maptools);library(geosphere) #gebruik R 3.3.5 (nodig voor polygonICES)
library(rgeos); library(RColorBrewer);library(GISTools)  

###############################
#1. Folders
#inPath    <- "W:/IMARES/Data/VMS_/2019/PMR_2019/Analyse/R_VMS_tbv_PercVis/Output/Script1.2/"
inPath    <- "W:/IMARES/Data/PMR/PMR 2019/Perceel Vis/3. data/VMS/"

#outPath   <- "W:/IMARES/Data/VMS_/2019/PMR_2019/Analyse/R_VMS_tbv_PercVis/Output/Script1.3/"
#shapePath <- "W:/IMARES/Data/VMS_/2019/PMR_2019/Analyse/R_VMS_tbv_PercVis/ShapeDFS"
shapePath <- "W:/IMARES/Data/PMR/PMR 2019/Perceel Vis/3. data/ICES_gebieden/ICES_gebieden_V7_WGS84/"

shapePMR  <- "W:/IMARES/Data/VMS_/2019/PMR_2019/Analyse/R_VMS_tbv_PercVis/ShapePMR"
shapeEuro <- "W:/IMARES/Data/VMS_/2019/PMR_2019/Analyse/R_VMS_tbv_PercVis/ShapeEuro/"
...
for (iG in c('Gar','BkorG','Pkor','Bkor','Bor','Dreg','Ander'))
{
...
#alleen surface per year
grd@data$Gar_tot <- apply(grd@data[,substr(names(grd@data),1,(5+nchar(iG))) %in% paste(iG,'_surf',sep='')],1,sum,na.rm=T)
#maak catergorien + legenda
if(iG == 'Gar')
{
SQcol <- c(-1,0,c(0.5, 1, 5, 10, 20, 40, 60, 1000))
legval   <- list(ALL = c("0",
                         "0 <= 0.5"  ,"0.5 <= 1.00"  ,"1.00 <= 5.00","5.00 <= 10",
...
cols     <- Color[cut(grd@data[,which(names(grd@data) == 'Gar_tot')],breaks=SQcol)]
...
print(paste(iG,iY))
  ...
  plot(density(grd@data$Gar_tot[grd@data$Gar_tot>0]))
  abline(v=seq(0,200,20))

...
```{r, echo=TRUE}
computer<-"Geert"
if (computer=="Geert") {path<-"c:/github/CoastalFish/fish_book/data";
                        env.path<-"C:/Users/gaarts/Dropbox/Seal_database/environmental/"}
...
# 
if (is.element("depth_raster.rdata",dir(file.path(path,"depth/Emodnet_DTM2018/")))){
  load(file.path(path,"depth/Emodnet_DTM2018/depth_raster.rdata"))}

if (is.element("depth_raster.rdata",dir(file.path(path,"depth/Emodnet_DTM2018/")))==FALSE){
...
# Calculate means of depth  
  ...
  M.raster<-raster(M,xmn=min(xy$x), xmx=max(xy$x), ymn=min(xy$y), ymx=max(xy$y))
    ...
    # Write.table
  save(M.raster,file=file.path(path,"depth/Emodnet_DTM2018/depth_raster.rdata"))

...
```{r extract_NETcdf, echo=TRUE, eval=T}
...
# Create empty matrix to store results  
  empty.dat<-as.data.frame(matrix(NA,nrow=nrow(dat),ncol=length(files)))
  names(empty.dat)<-the.col.names
...
for (the.year in 2005:2018)
{
  ...
  # Define the files for that year
    files<-paste(abiotiek$parameter,abiotiek$period,
                 abiotiek$layer10,abiotiek$aggregation,"_",the.year,abiotiek$veld,".nc",sep="")
    files<-files[is.element(files,dir(local.nc.path))]
  ...
  # Loop through all the files
    ...
    # Transform to utm coordinates  
      ...
      xy<-xytransform(table=xy,CRSold="+proj=longlat +datum=WGS84",CRSnew=common.projection,
                      xold="lon",yold="lat",xnew="x_utm",ynew="y_utm")
      xy$id<-1:nrow(xy)
      xy<-xy[is.na(xy$lon)==FALSE,]
  
    # fast nearest neighbour search
      closest <- nn2(xy[,3:4],cbind(dat.y$x_utm,dat.y$y_utm),k = 1, searchtype = "radius", 
                     radius = 5000)
      closest$nn.idx[closest$nn.idx==0]<-NA
      dat$layer.idx<-xy$id[closest$nn.idx]
      dat$layer.dist<-closest$nn.dists

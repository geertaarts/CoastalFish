...
Stack1 <- inla.stack(
  tag  = "Fit",
  data = list(y = dat.s$count[fit_ind],offset.arg=dat.s$the.offset[fit_ind]),    
  A    = list(1,1, A1),         
  effects = list( 
    Intercept=rep(1,N),
...
Stack2 <- inla.stack(
  tag  = "pred",
  data = list(y = NA,offset.arg=dat.s$the.offset[-fit_ind]),    
  A    = list(1,1, A2),         
  effects = list( 
    Intercept=rep(1,N),
    X=X.s[-fit_ind,], #Covariates
...
#fsp <- parse(text=c("y ~ -1 + Intercept + ",paste(c(names(X)[!names(X)=="the.offset"][1:5],"offset(the.offset2)"," f(w, model = spde)"),collapse =" + ")))
fsp <- parse(text=c("y ~ -1 + Intercept + ",paste(c(names(X)[!names(X)=="the.offset"][1:5]," f(w, model = spde)"),collapse =" + ")))
